...
def test_remote_policy_rejects_non_cool_without_contacting_controller(tmp_path) -> None:
    class Controller:
        calls = 0

        def set_state(self, state: AcState) -> CommandResult:
            self.calls += 1
            return CommandResult(CommandOutcome.CONFIRMED_SUCCESS)

        def power_off(self) -> CommandResult:
            self.calls += 1
            return CommandResult(CommandOutcome.CONFIRMED_SUCCESS)

    database = tmp_path / "remote-policy.db"
    run_migrations(database)
    controller = Controller()
    with SqliteCommandAuditRepository(database) as repository:
        execution = ExecuteCoolOnlyCommand(
            CommandService(
                device_id="node-1",
                controller=controller,
    ...
    assert execution.result.outcome is CommandOutcome.REJECTED_LOCALLY
    assert execution.result.error_message == "remote controls currently authorize only cool mode"
